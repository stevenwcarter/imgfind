...
use crate::grid_index::{CursorIndex, GridCols, ItemCount};
...
pub fn move_selection(
    ...
    let cols = cols.get().max(1);
    let i = match cur {
        None => return Some(0),
        Some(i) => i.get().min(len - 1),
    };
    let new = match dir {
...
mod tests {
    ...
    fn no_selection_any_direction_selects_first() {
        for dir in [NavDir::Left, NavDir::Right, NavDir::Up, NavDir::Down] {
            assert_eq!(
                move_selection(None, dir, GridCols(COLS), ItemCount(LEN)),
                Some(0)
            );
        }
    }
    ...
    fn empty_grid_returns_none() {
        ...
        assert_eq!(
            move_selection(None, NavDir::Right, GridCols(COLS), ItemCount(0)),
            None
        );
    }
    ...
    fn single_column_up_down_behave_like_prev_next() {
        ...
        assert_eq!(
            move_selection(Some(CursorIndex(2)), NavDir::Up, GridCols(1), ItemCount(5)),
            Some(1)
        );
        assert_eq!(
            move_selection(
        ...
        assert_eq!(
            move_selection(Some(CursorIndex(0)), NavDir::Up, GridCols(1), ItemCount(5)),
            Some(0)
        );
        assert_eq!(
            move_selection(
